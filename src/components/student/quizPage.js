...
let QuizPage = () => {
  ...
  useState(() => {
    document.addEventListener("visibilitychange", () => {
      document.title =
        document.visibilityState == "hidden" ? "cheating" : "Quiz";
      if (document.visibilityState == "hidden")
        window.alert("Your activity is being Monitered !!");
          ...
          <div className={styles.quizPanel}>
            <div className={styles.quizFrame}>
              <QuizComp
                ...
                quesStatement={
                  data.length ? data[index].question : "Loading..."
                }
              />
              {data.length
                ? data[index].options.map((a) => (
                    <Option
                      select={input[data[index]._id]}
                      index={option_indx++}
                      que={data[index]._id}
                      click={handelClick3}
                      key={a._id}
